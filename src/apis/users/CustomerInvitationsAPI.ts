...
export class CustomerInvitationsAPI extends BaseAPI {
   ...
   async filter (query?: { storeKey?: string, interfaceKey?: string, currencyCode?: string, languageCode?: string, priceListKey?: string, merchantKey?: string, expand?: string, project?: string, filter?: string, sort?: string, limit?: number, offset?: number }): Promise<CustomerInvitationFilterResultDTO> {
       const response = await this._request({
           path: `/users/customer-invitations`,
           method: 'GET',
