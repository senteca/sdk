...
export class ScriptsAPI extends BaseAPI {
   ...
   async filter (query?: { storeKey?: string, interfaceKey?: string, currencyCode?: string, languageCode?: string, priceListKey?: string, merchantKey?: string, expand?: string, project?: string, filter?: string, sort?: string, limit?: number, offset?: number }): Promise<ScriptFilterResultDTO> {
       const response = await this._request({
           path: `/config/scripts`,
           method: 'GET',
