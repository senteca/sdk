...
export class StatesAPI extends BaseAPI {
   ...
   async filter (query?: { storeKey?: string, currencyCode?: string, languageCode?: string, priceListKey?: string, time?: number, merchantKey?: string, expand?: string, project?: string, filter?: string, sort?: string, limit?: number, offset?: number }): Promise<StateFilterResultDTO> {
       const response = await this._request({
           path: `/config/states`,
           method: 'GET',
