...
export class ShippingConfigsAPI extends BaseAPI {
   ...
   async filter (query?: { storeKey?: string, interfaceKey?: string, currencyCode?: string, languageCode?: string, priceListKey?: string, merchantKey?: string, expand?: string, project?: string, filter?: string, sort?: string, limit?: number, offset?: number }): Promise<ShippingConfigFilterResultDTO> {
       const response = await this._request({
           path: `/fulfillment/shipping-configs`,
           method: 'GET',
